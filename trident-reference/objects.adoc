...
NOTE: 一般而言、您不需要管理 `VolumeSnapshotContent`物件。例外情況是您想要link:../trident-use/vol-snapshots.html#import-a-volume-snapshot["匯入 Volume 快照"]在 Astra Trident 之外建立。
...
如果您偏好控制自己的資源配置、可以使用命令移除 Astra Trident `ResourceQuota` 物件：
