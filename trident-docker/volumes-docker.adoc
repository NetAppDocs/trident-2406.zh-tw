...
summary: '若要建立 Volume 、請使用標準命令、 `docker volume`並在需要時指定 Astra Trident 驅動程式名稱。' 
---
= 使用Volume
:hardbreaks:
:allow-uri-read: 
...
使用 `ontap-nas`、 `ontap-san`、 `solidfire-san`和 `gcp-cvs storage drivers`時， Astra Trident 可以複製卷。使用 `ontap-nas-flexgroup`或 `ontap-nas-economy`驅動程式時、不支援複製。從現有磁碟區建立新磁碟區、將會產生新的快照。
